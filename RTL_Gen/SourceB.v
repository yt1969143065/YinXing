module SourceB(
  ...
  input  [1:0]   io_task_bits_alias_0,
  input  [1:0]   io_task_bits_alias_1,
...
`endif // RANDOMIZE_REG_INIT
  ...
  wire [1:0] chosenClient = pendingClient[0] ? 2'h1 : 2'h2; // @[ParallelMux.scala 92:77]
  ...
  reg [7:0] taskLatch_rset; // @[Reg.scala 16:16]
  reg [20:0] taskLatch_rtag; // @[Reg.scala 16:16]
  reg [2:0] taskLatch_rparam; // @[Reg.scala 16:16]
  reg [1:0] taskLatch_ralias_0; // @[Reg.scala 16:16]
  reg [1:0] taskLatch_ralias_1; // @[Reg.scala 16:16]
  reg  taskLatch_rneedData; // @[Reg.scala 16:16]
  wire [7:0] taskLatch_set = _io_task_ready_T ? io_task_bits_set : taskLatch_rset; // @[SourceB.scala 50:22]
  wire [20:0] taskLatch_tag = _io_task_ready_T ? io_task_bits_tag : taskLatch_rtag; // @[SourceB.scala 50:22]
  wire [2:0] taskLatch_param = _io_task_ready_T ? io_task_bits_param : taskLatch_rparam; // @[SourceB.scala 50:22]
  wire [1:0] taskLatch_alias_0 = _io_task_ready_T ? io_task_bits_alias_0 : taskLatch_ralias_0; // @[SourceB.scala 50:22]
  wire [1:0] taskLatch_alias_1 = _io_task_ready_T ? io_task_bits_alias_1 : taskLatch_ralias_1; // @[SourceB.scala 50:22]
  wire  taskLatch_needData = _io_task_ready_T ? io_task_bits_needData : taskLatch_rneedData; // @[SourceB.scala 50:22]
  wire [1:0] _probe_alias_T_2 = chosenClient[0] ? taskLatch_alias_0 : 2'h0; // @[Mux.scala 27:73]
  wire [1:0] _probe_alias_T_3 = chosenClient[1] ? taskLatch_alias_1 : 2'h0; // @[Mux.scala 27:73]
  wire [1:0] probe_alias = _probe_alias_T_2 | _probe_alias_T_3; // @[Mux.scala 27:73]
  wire [34:0] _io_baddress_T = {taskLatch_tag,taskLatch_set,6'h0}; // @[Cat.scala 31:58]
  wire [2:0] _io_bdata_T = {probe_alias,taskLatch_needData}; // @[Cat.scala 31:58]
  assign io_bvalid = busy | io_task_valid; // @[SourceB.scala 52:22]
  assign io_bparam = taskLatch_param[1:0]; // @[SourceB.scala 54:19]
  assign io_bsource = chosenClient[1] ? 6'h28 : 6'h0; // @[Mux.scala 27:73]
  assign io_baddress = {{1'd0}, _io_baddress_T}; // @[SourceB.scala 57:21]
  ...
  always @(posedge clock) begin
    if (reset) begin // @[SourceB.scala 35:24]
      workVec <= 2'h0; // @[SourceB.scala 35:24]
    end else if (_T_1) begin // @[SourceB.scala 45:19]
      workVec <= _workVec_T_1; // @[SourceB.scala 46:13]
    ...
    if (_T) begin // @[Reg.scala 17:18]
      taskLatch_ralias_0 <= io_task_bits_alias_0; // @[Reg.scala 17:22]
    end
    if (_T) begin // @[Reg.scala 17:18]
      taskLatch_ralias_1 <= io_task_bits_alias_1; // @[Reg.scala 17:22]
    end
    if (_T) begin // @[Reg.scala 17:18]
...
`ifdef RANDOMIZE_REG_INIT
  ...
  workVec = _RAND_0[1:0];
  _RAND_1 = {1{`RANDOM}};
  taskLatch_rset = _RAND_1[7:0];
  _RAND_2 = {1{`RANDOM}};
  taskLatch_rtag = _RAND_2[20:0];
  _RAND_3 = {1{`RANDOM}};
  taskLatch_rparam = _RAND_3[2:0];
  _RAND_4 = {1{`RANDOM}};
  taskLatch_ralias_0 = _RAND_4[1:0];
  _RAND_5 = {1{`RANDOM}};
  taskLatch_ralias_1 = _RAND_5[1:0];
  _RAND_6 = {1{`RANDOM}};
  taskLatch_rneedData = _RAND_6[0:0];
`endif // RANDOMIZE_REG_INIT
  `endif // RANDOMIZE
end // initial
