module FMUL_s1(
  ...
  wire [8:0] exp_sum = raw_a_exp + raw_bexp; // @[FMUL.scala 77:27]
  wire [8:0] prod_exp = exp_sum - 9'h64; // @[FMUL.scala 78:26]
  wire [9:0] _shift_lim_sub_T = {1'h0,exp_sum}; // @[Cat.scala 31:58]
  wire [9:0] shift_lim_sub = _shift_lim_sub_T - 10'h65; // @[FMUL.scala 80:46]
  wire  prod_exp_uf = shift_lim_sub[9]; // @[FMUL.scala 81:39]
  wire [8:0] shift_lim = shift_lim_sub[8:0]; // @[FMUL.scala 82:37]
  wire [23:0] subnormal_sig = decode_a_expIsZero ? raw_a_sig : raw_bsig; // @[FMUL.scala 87:26]
  wire [8:0] _GEN_2 = {{3'd0}, lzc_clz_io_out}; // @[FMUL.scala 89:30]
  wire  exceed_lim = shift_lim <= _GEN_2; // @[FMUL.scala 89:30]
  wire [8:0] _shift_amt_T = exceed_lim ? shift_lim : {{3'd0}, lzc_clz_io_out}; // @[FMUL.scala 90:44]
  wire [8:0] shift_amt = prod_exp_uf ? 9'h0 : _shift_amt_T; // @[FMUL.scala 90:22]
  ...
  assign io_out_early_overflow = exp_sum > 9'h17d; // @[FMUL.scala 84:29]
  assign io_out_prod_sign = fp_a_sign ^ fp_bsign; // @[FMUL.scala 60:29]
  assign io_out_shift_amt = prod_exp_uf ? 9'h0 : _shift_amt_T; // @[FMUL.scala 90:22]
