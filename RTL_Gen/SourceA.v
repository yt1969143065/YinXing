...
`endif // RANDOMIZE_REG_INIT
  ...
  wire  a_acquire_valid = io_task_valid & ~io_task_bits_putData; // @[SourceA.scala 60:36]
  reg  s1_full; // @[SourceA.scala 63:24]
  ...
  wire  _T_1 = _T & io_task_bits_putData; // @[SourceA.scala 46:24]
  wire  _GEN_0 = _T & io_task_bits_putData | busy; // @[SourceA.scala 46:49 47:10 42:21]
  wire [34:0] _a_acquire_bits_address_T = {io_task_bits_tag,io_task_bits_set,6'h0}; // @[Cat.scala 31:58]
  reg [20:0] s0_task_tag; // @[Reg.scala 16:16]
  reg [7:0] s0_task_set; // @[Reg.scala 16:16]
  ...
  reg  s0_count; // @[SourceA.scala 67:25]
  wire  s0_last = s0_task_size == 3'h6 ? s0_count : ~s0_count; // @[SourceA.scala 70:20]
  ...
  wire  s1_cango = s1_full & out_ready; // @[SourceA.scala 90:21]
  wire  s1_ready = s1_cango | ~s1_full; // @[SourceA.scala 93:24]
  wire  s1_latch = s0_valid & s1_ready; // @[SourceA.scala 87:27]
  reg [20:0] s1_task_tag; // @[Reg.scala 16:16]
  reg [7:0] s1_task_set; // @[Reg.scala 16:16]
  ...
  reg  s1_pb_latch_REG; // @[SourceA.scala 91:51]
  reg [255:0] s1_pb_latch_rdata; // @[Reg.scala 28:20]
  reg [31:0] s1_pb_latch_rmask; // @[Reg.scala 28:20]
  wire [31:0] _GEN_27 = s1_pb_latch_REG ? io_pb_beat_mask : s1_pb_latch_rmask; // @[Reg.scala 29:18 28:20 29:22]
  wire [255:0] _GEN_28 = s1_pb_latch_REG ? io_pb_beat_data : s1_pb_latch_rdata; // @[Reg.scala 29:18 28:20 29:22]
  wire  _GEN_29 = s1_full & s1_cango ? 1'h0 : s1_full; // @[SourceA.scala 63:24 95:{29,39}]
  wire  _GEN_30 = s1_latch | _GEN_29; // @[SourceA.scala 96:{18,28}]
  wire [34:0] _a_put_bits_address_T = {s1_task_tag,s1_task_set,s1_task_off}; // @[Cat.scala 31:58]
  ...
  wire [31:0] _T_36 = muxStateEarly_0 ? _GEN_27 : 32'h0; // @[Mux.scala 27:73]
  wire [31:0] _T_37 = muxStateEarly_1 ? 32'hffffffff : 32'h0; // @[Mux.scala 27:73]
  wire [35:0] a_put_bits_address = {{1'd0}, _a_put_bits_address_T}; // @[SourceA.scala 40:19 102:22]
  wire [35:0] _T_45 = muxStateEarly_0 ? a_put_bits_address : 36'h0; // @[Mux.scala 27:73]
  wire [35:0] a_acquire_bits_address = {{1'd0}, _a_acquire_bits_address_T}; // @[SourceA.scala 39:23 54:26]
  wire [35:0] _T_46 = muxStateEarly_1 ? a_acquire_bits_address : 36'h0; // @[Mux.scala 27:73]
  wire [3:0] _T_48 = muxStateEarly_0 ? s1_task_source : 4'h0; // @[Mux.scala 27:73]
  wire [3:0] _T_49 = muxStateEarly_1 ? io_task_bits_source : 4'h0; // @[Mux.scala 27:73]
  wire [2:0] _T_51 = muxStateEarly_0 ? s1_task_size : 3'h0; // @[Mux.scala 27:73]
  wire [2:0] _T_52 = muxStateEarly_1 ? 3'h6 : 3'h0; // @[Mux.scala 27:73]
  wire [2:0] _T_54 = muxStateEarly_0 ? s1_task_param : 3'h0; // @[Mux.scala 27:73]
  wire [2:0] _T_55 = muxStateEarly_1 ? io_task_bits_param : 3'h0; // @[Mux.scala 27:73]
  wire [2:0] _T_57 = muxStateEarly_0 ? s1_task_opcode : 3'h0; // @[Mux.scala 27:73]
  wire [2:0] _T_58 = muxStateEarly_1 ? io_task_bits_opcode : 3'h0; // @[Mux.scala 27:73]
  assign io_a_valid = idle ? _T_16 : _sink_ACancel_earlyValid_T_3; // @[Arbiter.scala 125:29]
  assign io_a_bits_opcode = _T_57 | _T_58; // @[Mux.scala 27:73]
  assign io_a_bits_param = _T_54 | _T_55; // @[Mux.scala 27:73]
  assign io_a_bits_size = _T_51 | _T_52; // @[Mux.scala 27:73]
  assign io_a_bits_source = _T_48 | _T_49; // @[Mux.scala 27:73]
  assign io_a_bits_address = _T_45 | _T_46; // @[Mux.scala 27:73]
  assign io_a_bits_mask = _T_36 | _T_37; // @[Mux.scala 27:73]
  assign io_a_bits_data = muxStateEarly_0 ? _GEN_28 : 256'h0; // @[Mux.scala 27:73]
  assign io_task_ready = io_task_bits_putData ? ~busy : out_1_ready; // @[SourceA.scala 44:23]
  assign io_pb_pop_valid = busy & s1_ready; // @[SourceA.scala 73:27]
  assign io_pb_pop_bits_bufIdx = s0_task_bufIdx; // @[SourceA.scala 74:25]
  assign io_pb_pop_bits_count = s0_count; // @[SourceA.scala 75:24]
  assign io_pb_pop_bits_last = s0_task_size == 3'h6 ? s0_count : ~s0_count; // @[SourceA.scala 70:20]
  always @(posedge clock) begin
    if (reset) begin // @[SourceA.scala 42:21]
      busy <= 1'h0; // @[SourceA.scala 42:21]
    end else if (s0_valid) begin // @[SourceA.scala 78:27]
      if (s0_last) begin // @[SourceA.scala 80:20]
        busy <= 1'h0; // @[SourceA.scala 81:12]
      end else begin
        busy <= _GEN_0;
      end
    end else begin
    ...
    if (reset) begin // @[SourceA.scala 63:24]
      s1_full <= 1'h0; // @[SourceA.scala 63:24]
    end else begin
      s1_full <= _GEN_30;
    end
    ...
    if (reset) begin // @[SourceA.scala 67:25]
      s0_count <= 1'h0; // @[SourceA.scala 67:25]
    end else if (s0_valid) begin // @[SourceA.scala 78:27]
      if (s0_last) begin // @[SourceA.scala 80:20]
        s0_count <= 1'h0; // @[SourceA.scala 82:16]
      end else begin
        s0_count <= s0_count + 1'h1; // @[SourceA.scala 79:14]
      end
    end
    if (reset) begin // @[Arbiter.scala 116:26]
    ...
    s1_pb_latch_REG <= s0_valid & s1_ready; // @[SourceA.scala 87:27]
    if (reset) begin // @[Reg.scala 28:20]
      s1_pb_latch_rdata <= 256'h0; // @[Reg.scala 28:20]
    end else if (s1_pb_latch_REG) begin // @[Reg.scala 29:18]
      s1_pb_latch_rdata <= io_pb_beat_data; // @[Reg.scala 29:22]
    end
    if (reset) begin // @[Reg.scala 28:20]
      s1_pb_latch_rmask <= 32'h0; // @[Reg.scala 28:20]
    end else if (s1_pb_latch_REG) begin // @[Reg.scala 29:18]
      s1_pb_latch_rmask <= io_pb_beat_mask; // @[Reg.scala 29:22]
    end
  end
...
`ifdef RANDOMIZE_REG_INIT
  ...
  state_1 = _RAND_3[0:0];
  _RAND_4 = {1{`RANDOM}};
  s0_task_tag = _RAND_4[20:0];
  _RAND_5 = {1{`RANDOM}};
  s0_task_set = _RAND_5[7:0];
  _RAND_6 = {1{`RANDOM}};
  ...
  _RAND_14 = {1{`RANDOM}};
  s1_task_tag = _RAND_14[20:0];
  _RAND_15 = {1{`RANDOM}};
  s1_task_set = _RAND_15[7:0];
  _RAND_16 = {1{`RANDOM}};
